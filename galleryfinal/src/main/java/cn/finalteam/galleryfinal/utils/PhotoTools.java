...
public class PhotoTools {
    ...
    public static List<PhotoFolderInfo> getAllPhotoFolder(Context context, List<PhotoInfo> selectPhotoMap) {
        ...
        List<PhotoInfo> selectedList = GalleryFinal.getFunctionConfig().getSelectedList();
                ...
                while (cursor.moveToNext()) {
                    ...
                    if ( (filterList == null || !filterList.contains(path)) && file.exists() && file.length() > 0 ) {
                        ...
                        if (GalleryFinal.getFunctionConfig().isCamera()) {
                            photoInfo.setPosition(position + 1);
                        } else {
...
//                        }
                        if (selectedList != null && selectedList.size() > 0 && isPhotoHasSelected(photoInfo, selectedList, path)) {
                            selectPhotoMap.add(photoInfo);
                        }
                    }
                }
    ...
    private static boolean isPhotoHasSelected(PhotoInfo photoInfo, List<PhotoInfo> selectedList, String photoPath) {
        for (int i = 0; i < selectedList.size(); i++) {
            PhotoInfo pi = selectedList.get(i);
            if (pi.getPhotoPath().equals(photoPath)) {
                photoInfo.setIndex(pi.getIndex());
                return true;
            }
        }
        return false;
