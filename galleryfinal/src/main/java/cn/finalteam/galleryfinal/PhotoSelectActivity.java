...
//        mFabOk.setColorNormal(GalleryFinal.getGalleryTheme().getFabNornalColor());
    ...
    private void takeRefreshGallery(PhotoInfo photoInfo) {
        if (mSelectPhotoList != null) {
            photoInfo.setIndex(mSelectPhotoList.size());
        }
        if (GalleryFinal.getFunctionConfig().isCamera()) {
            photoInfo.setPosition(1);
            mCurPhotoList.add(1, photoInfo); // 因为第一个位置已经被camera功能图标占领了
        } else {
            photoInfo.setPosition(0);
    ...
    protected void takeResult(PhotoInfo photoInfo) {
        ...
        message.what = HANLDER_TAKE_PHOTO_EVENT;
        photoInfo.setIndex(mSelectPhotoList.size());
        photoInfo.setPosition(1); // 放在第一张图位置
        ...
        // 除了选中的第一张，其他positon都加一
        for (int i = 1; i < mSelectPhotoList.size(); i++) {
            int position = mSelectPhotoList.get(i).getPosition();
            mSelectPhotoList.get(i).setPosition(position + 1);
        }
    }
    ...
    private void folderItemClick(int position) {
        ...
        if (GalleryFinal.getFunctionConfig().isCamera()) {
            // 添加拍照item
            mCurPhotoList.add(0, new PhotoInfo());
        }
    ...
    public void addNumberView(TextView tvNumberView) {
        if (tvNumberList == null) {
            tvNumberList = new ArrayList<>();
        }
        if (!tvNumberList.contains(tvNumberView)) {
            tvNumberList.add(tvNumberView);
        }
    }

    public void removeNumberView(TextView tvNumberView) {
        if (tvNumberList != null) {
            tvNumberList.remove(tvNumberView);
        }
    }

    public void clearAllNumberView() {
        if (tvNumberList != null) {
            tvNumberList.clear();
        }
    }
    ...
    private void photoItemClick(View view, int position) {
        ...
        if (!mSelectPhotoList.contains(info)) {
            if (GalleryFinal.getFunctionConfig().isMutiSelect() && mSelectPhotoList.size() == GalleryFinal.getFunctionConfig().getMaxSize()) {
                ...
                mSelectPhotoList.add(info);
                info.setIndex(mSelectPhotoList.size());
                checked = true;
            }
        } else {
            try {
        ...
        if (!checked) {
            // 重新排序，所有大于删除图片的index都减一
            for (int i = 0; i < mSelectPhotoList.size(); i++) {
                int index = mSelectPhotoList.get(i).getIndex();
                if (index > removedItemIndex) {
                    mSelectPhotoList.get(i).setIndex(index - 1);
                }
            }
        }
        refreshSelectCount();
...
//                holder.mIvCheck.setBackgroundColor(GalleryFinal.getGalleryTheme().getCheckSelectedColor());
                ...
                holder.mTvNumber.setText(mSelectPhotoList.size() + "");
            } else {
//                holder.mIvCheck.setBackgroundColor(GalleryFinal.getGalleryTheme().getCheckNornalColor());
                ...
                // 重新排序，所有大于删除图片的index都减一
                for (int i = 0; i < mSelectPhotoList.size(); i++) {
                    int photoPosition = mSelectPhotoList.get(i).getPosition();
                    View photoView = mGvPhotoList.getChildAt(photoPosition);
                    if (photoView != null) {
                        View vnumber = photoView.findViewById(R.id.tv_number);
                        if (vnumber != null) {
                            TextView tvNumber = (TextView) vnumber;
                            int number = Integer.parseInt(tvNumber.getText().toString());
                            if (number > removedItemIndex) {
